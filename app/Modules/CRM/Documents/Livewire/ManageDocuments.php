...
namespace App\Modules\CRM\Documents\Livewire;
...
class ManageDocuments extends Component
{
    ...
    public function bulkDelete(): void
    {
        ...
        $docs = Document::whereIn('id', $this->selectedDocs)->get();
        foreach ($docs as $doc) {
            if ($doc->file_path && Storage::disk('public')->exists($doc->file_path)) {
                Storage::disk('public')->delete($doc->file_path);
            }
    ...
    {
        $query = Document::with(['client.user', 'website', 'addedBy']);
        ...
        $clients = Client::with('user')->orderBy(
            \App\Models\User::select('name')
                ->whereColumn('users.id', 'adspv_clients.user_id')
        )->get();
        ...
        return view('modules.crm.documents.manage-documents', [
            'documents' => $documents,
            'clients' => $clients,
            'websites' => $websites,
