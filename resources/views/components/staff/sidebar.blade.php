...
<aside :class="mobileSidebar ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 w-64 glass border-r border-slate-200/50 dark:border-slate-800/50 z-40 transition-transform duration-300 lg:translate-x-0 flex flex-col">
    ...
    <div class="h-16 flex items-center justify-between px-6 border-b border-slate-200/50 dark:border-slate-800/50">
        <a href="{{ route('staff.dashboard') }}" class="flex items-center py-1">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-6 space-y-1.5 custom-scrollbar"
         x-data="{ init() { 
             $nextTick(() => { this.$el.scrollTop = sessionStorage.getItem('staff_sidebar_scroll') || 0; });
             this.$el.addEventListener('scroll', () => { 
                 sessionStorage.setItem('staff_sidebar_scroll', this.$el.scrollTop); 
             }); 
         } }">
        @php
            $navItems = [
                ...
                ['route' => 'staff.documents', 'label' => 'Resources Library', 'icon' => 'M8 7v12m0 0l-4-4m4 4l4-4m0 6h8a2 2 0 002-2V7a2 2 0 00-2-2h-8a2 2 0 00-2 2v10a2 2 0 00-2 2z'],
                ['route' => 'staff.support', 'label' => 'Support Center', 'icon' => 'M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z'],
                ['route' => 'staff.media', 'label' => 'Media', 'icon' => 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z'],
        ...
        @foreach($navItems as $item)
            @php
                $route = $item['route'];
                $isActive = request()->routeIs($route) || request()->routeIs($route . '.*') || request()->routeIs($route . '-*') || (request()->is('staffadspnl/' . strtolower(explode(' ', $item['label'])[0]) . '*'));
