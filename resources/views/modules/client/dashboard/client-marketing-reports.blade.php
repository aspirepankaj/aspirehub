<div class="space-y-6">
    ...
    @if($activeReportIntegrationId === 'overview')
        @if($isGa4Connected || $isGscConnected || $isYoutubeConnected || $isKeywordConnected)
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" wire:key="overview-grid-{{ $dateFrom }}-{{ $dateTo }}">
                <!-- GA4 Metrics -->
                @if($hasGa4)
                    @php 
            ...
            <!-- Tables Row -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" wire:key="overview-tables-{{ $dateFrom }}-{{ $dateTo }}">
    ...
    @if($activeReportIntegrationId === 'ga4')
        @if($hasGa4)
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-xs font-bold flex items-center gap-2">
                                <span class="text-white">{{ ucfirst(explode('-', $dateFrom)[1] ?? '') }} {{ explode('-', $dateFrom)[0] ?? '' }}</span>
            ...
            @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6" wire:key="ga4-grid-{{ $dateFrom }}-{{ $compareMonth }}">
                <div class="bg-white dark:bg-slate-950/40 border border-slate-100 dark:border-slate-900 p-5 rounded-2xl shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
    ...
    @if($activeReportIntegrationId === 'gsc')
        @if($hasGsc)
            ...
            @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6" wire:key="gsc-grid-{{ $dateFrom }}-{{ $compareMonth }}">
                <div class="bg-white dark:bg-slate-950/40 border border-slate-100 dark:border-slate-900 p-5 rounded-2xl shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
    ...
    @elseif ($activeReportIntegrationId === 'youtube')
        @if($hasYoutube)
            ...
            @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6" wire:key="youtube-grid-{{ $dateFrom }}-{{ $compareMonth }}">
    ...
    @elseif ($activeReportIntegrationId === 'keyword')
        @if($hasKeyword)
            ...
            @endphp
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6" wire:key="keyword-grid-{{ $dateFrom }}-{{ $compareMonth }}">
                <div class="p-5 bg-white dark:bg-slate-950/40 border border-slate-100 dark:border-slate-900 rounded-2xl shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
