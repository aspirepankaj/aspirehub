<div>
    ...
    @if (session()->has('error'))
        <div class="mb-6 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs sm:text-sm font-semibold flex items-center gap-2">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <!-- Search -->
        <div class="relative flex items-center flex-1 max-w-md w-full">
            ...
            <input type="text" wire:model.live.debounce.350ms="search" placeholder="Search resources..."
                class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 text-slate-900 dark:text-white text-xs sm:text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-500/40" />
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse ($documents as $doc)
            ...
            @endphp
            <div class="bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-200 dark:border-slate-800/60 p-4 flex items-center justify-between shadow-sm hover:border-slate-350 dark:hover:border-slate-750 transition duration-150 overflow-hidden">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    <!-- Icon based on extension -->
                    <div class="w-10 h-10 sm:w-11 sm:h-11 shrink-0 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-750/30 flex items-center justify-center text-slate-500 dark:text-slate-400">
                    ...
                    <div class="min-w-0 flex-1">
                        <h4 class="text-xs sm:text-sm font-bold text-slate-850 dark:text-slate-200 truncate" title="{{ $doc->title }}">
                            {{ $doc->title }}
                        </h4>
                        <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mt-1 truncate">
                            {{ $doc->category }} 
                            @if($doc->resource_type !== 'link')
                                &bull; {{ $sizeInMb }} MB 
                            @endif
                        ...
                        @if ($doc->website_name)
                            <p class="text-[10px] text-indigo-500 dark:text-indigo-400 font-semibold mt-0.5 truncate">
