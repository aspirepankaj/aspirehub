...
<div>
    {{-- Breadcrumbs --}}
    <x-admin.breadcrumbs :items="['Maintenance Reports' => route('admin.maintenance'), 'Report Details' => null]" />
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6 bg-white/40 dark:bg-slate-900/40 p-4 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.maintenance.edit', $report->id) }}"
               class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold transition active:scale-95 shadow-sm">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
