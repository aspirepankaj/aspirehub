...
<div>
        ...
        <div class="border-b border-slate-200/60 dark:border-slate-800/40 mb-6">
            <nav class="flex space-x-8" aria-label="Tabs">
                @foreach(['overview' => 'Overview', 'maintenance' => 'Maintenance Reports', 'activity log' => 'Activity Log'] as $tabKey => $tabLabel)
                    <a href="{{ route('staff.websites.detail', ['id' => $websiteDetails->id, 'tab' => $tabKey]) }}" wire:navigate
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-900/50 text-sm">
                        @foreach($websites as $website)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition-colors">
                                {{-- Website Name + URL --}}
                                <td class="px-4 py-4">
                                    <a href="{{ route('staff.websites.detail', ['id' => $website->id]) }}" wire:navigate class="font-bold text-slate-900 dark:text-white hover:text-indigo-600 dark:hover:text-indigo-400 transition text-left block">{{ $website->site_name }}</a>
    ...
    <x-admin.modal name="edit-website-modal" title="Edit Website" maxWidth="max-w-3xl">
        <form wire:submit.prevent="updateWebsite" class="space-y-4 mt-2">
            ...
            <div x-data="{
                get imgUrl() {
                    let img = $wire.image || $wire.existing_image;
                    if (!img) return null;
                ...
                <div class="mt-1.5 flex items-center gap-3">
                    ...
                    <button type="button" x-show="imgUrl" wire:click="removeImage" class="px-3 py-1.5 text-xs font-semibold text-red-600 bg-white border border-red-200 rounded-xl hover:bg-red-50 transition shadow-sm">
                        Remove
                    </button>
                </div>
                <x-input-error :messages="$errors->get('image')" class="mt-1" />
