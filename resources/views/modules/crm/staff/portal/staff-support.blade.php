    ...
    <div class="bg-white/60 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl p-5 shadow-sm flex items-center justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Manage and respond to support tickets raised by your assigned clients.
            </p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-[600px]">
        ...
        <div class="lg:col-span-4 space-y-4">
            <div class="bg-white/60 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl p-4 shadow-sm space-y-3">
                <div class="relative">
                    <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search ticket, client or subject..."
            ...
            <div class="space-y-2.5 max-h-[580px] overflow-y-auto pr-1">
                ...
                @empty
                    <div class="p-8 text-center bg-white/40 dark:bg-slate-900/30 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800">
                        <p class="text-xs font-semibold text-slate-500 dark:text-slate-400">No support tickets assigned.</p>
                    </div>
                @endforelse
            </div>
        </div>

        <!-- Right Column: Ticket Conversation & Status Controls (8 Columns) -->
        <div class="lg:col-span-8 flex flex-col">
            @if ($selectedTicket)
                <div class="bg-white/80 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800/60 rounded-2xl shadow-sm flex flex-col h-full overflow-hidden">
                    <!-- Ticket Header with Status Action Buttons -->
                    <div class="p-4 sm:p-5 border-b border-slate-200/60 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-900/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            ...
                            <select wire:change="updateStatus($event.target.value)" class="px-3 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500/40">
                                        ...
                                        <div class="flex items-center gap-2 {{ $isMe ? 'flex-row-reverse' : '' }}">
                                            <span class="text-[11px] font-bold text-slate-700 dark:text-slate-300">
                                                ...
                                                @if($msg->sender_type === 'admin')
                                                    <span class="text-[9px] px-1.5 py-0.2 rounded bg-purple-100 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300 font-extrabold ml-1">ADMIN</span>
                                                @elseif($msg->sender_type === 'client')
                                                    <span class="text-[9px] px-1.5 py-0.2 rounded bg-emerald-100 dark:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 font-extrabold ml-1">CLIENT</span>
                                                @endif
                                            </span>
                    ...
                    <div x-data="{
                        ...
                        <form wire:submit.prevent="sendReply" class="flex items-center gap-2">
                            ...
                            <input type="text" wire:model="replyMessage" placeholder="Type response or paste image (Ctrl+V / Cmd+V)..."
            ...
            @else
                <div class="bg-white/60 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/40 rounded-2xl p-12 text-center h-full flex flex-col items-center justify-center">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Select any ticket from the list to view conversation history and reply to client.</p>
                </div>
            @endif
        </div>
    </div>
