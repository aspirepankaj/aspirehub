    ...
    <div class="glass-card p-6 rounded-3xl border border-slate-200/50 dark:border-slate-800/50 shadow-xl relative overflow-hidden bg-white/60 dark:bg-slate-900/40 backdrop-blur-md">
        ...
        <div class="flex items-center space-x-4">
            @if(auth()->user()->staff?->profile_image)
                <img src="{{ Str::startsWith(auth()->user()->staff->profile_image, 'http') ? auth()->user()->staff->profile_image : asset('storage/' . auth()->user()->staff->profile_image) }}" alt="{{ auth()->user()->name }}" class="w-16 h-16 rounded-2xl object-cover border border-slate-200 dark:border-slate-800 shadow-lg shadow-pink-500/10" onerror="this.outerHTML=`<div class='w-16 h-16 rounded-2xl bg-gradient-to-tr from-pink-500 to-indigo-500 text-white flex items-center justify-center font-bold text-2xl shadow-lg shadow-pink-500/20'>{{ auth()->user()->getInitials() }}</div>`" />
            @else
